...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["Justificacion"]) && isset($_POST["idioma"]) && $_POST['idioma'] != "") {
    ...
    $nombreArchivo =  'ALUMNOS ATENDIDOS EN LENGUAS ACUMULADO';
    $idioma = $_POST['idioma'];
    ...
    if ($stmt === false) {
        ...
        if (archivoExistencia($nombreArchivo)) {
            ...
            if ($totalSheets > 1) {
                ...
                $rangoCeldas = 'B16:B27';
                guardarContenidoEnCelda($spreadsheet, 'J13', $corteActual,1);
                guardarContenidoEnCelda($spreadsheet, 'C13', $cortePasado,1);
                guardarContenidoEnCelda($spreadsheet, 'X7', $periodo,1);
                guardarContenidoEnCelda($spreadsheet, 'X8', $fechaCorte,1);

                $celdaR = ObtenerCeldaIdioma($rangoCeldas, $idioma, $hoja);

                if (is_numeric($celdaR)) {
                    guardarContenidoEnCelda($spreadsheet, 'T' . $celdaR, $justificacion,1);
